...
import { TextActiveH3, TextActiveH4, TextH4, TextH5, TextH6 } from "../../components/typography";
import { ActivePhoneSvg, ActiveRightIconSvg, ArrowRightIconSvg, meetNowImage1, meetNowImage2, meetNowImage3, meetNowImage4, meetNowUserImage1, meetNowUserImage2, meetNowUserImage3, meetNowUserImage4, PhoneIconSvg, PlusIconSvg, SortIconSvg, UserAvatarIconSvg } from "../../assets/image";
		...
		<React.Fragment>
			<Layout navigation={navigation} disableHeader={true}>
				<MeetNowContainer>
					...
					<SearchComponent title="鹿児島県" />

					<View>
						{MockMeetNowLists.map((data: MeetNowItemObject, key: number) => (
							<MeetNowItem navigation={navigation} data={data} key={key} />
						))}
					</View>
				</MeetNowContainer>
			</Layout>
...
const SearchComponent = ({ title }: SearchComponentProps) => {
	...
	return (
		<SearchComponentWrapper>
			<SearchTextContainer>
				...
				<ArrowRightIconSvg width={getWidth(6)} height={getWidth(6)} />
			</SearchTextContainer>
...
const MeetNowItem = ({ data, navigation }: MeetNowItemProps) => {
	const handleMore = () => {
		navigation.navigate(routerConfig.meetDetail)
	}

	return (
		<MeetItemWrapper>
			<View style={meetNowItemStyles.imageContainer}>
				<Image source={data.image} style={meetNowItemStyles.image} />
			</View>
...
const SearchComponentWrapper = styled(View)(({ theme }) => ({
	display: "flex",
	flexDirection: "row",
	alignItems: "center",
